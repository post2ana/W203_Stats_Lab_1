...
3. 10 companies with negative profits is worth investigating. Since most of the profits is from zero to 250 million that bin is worth some further analysis. We may split this bucket in future to see if there are any trends.

4. It will be interesting to see the salaries of CEOs with negative profit for 8 companies in 0 and -200 million range. 
...
```{r}
cleanCEO$grad <- factor(cleanCEO$grad,
...
2. This means that the data is pretty well distributed.
...
is any relationship between the profit that companies had and the salaries of CEOs.

Seeing the high clustering of profits data in the scatterplotmatrix we were tempted to take a log value of profits and plot against salary to look for an exponential relationship. Due to the presence of 10 negative profit values, we stayed away from taking a log value as this would remove the negative values from the analysis.

We tried to bin the profits data into and see what the salaries for CEOs were for individual buckets. We also wanted to  look for trends in salary as profit bins changed.

Histogram analysis of profits done earlier led us to create a bin for negative profits. Since most of the values were in the 0 to 250 million range, it made sense
to split this range to do a more in-depth analysis. Hence we created two more bins of profits in the range of 0-125 million and 125-250 million. Finally, we created another bin of values greater than 250 million to bundle higher values of profit together.

```{r}
profit_bin = cut(cleanCEO$profits, breaks = c(-500,0,125,250, Inf), labels = 
      c("Negative Profits", "Profits < 125","125 < Profits < 250", "Profits > 250"))
summary(profit_bin)
```

```{r}
boxplot(salary/1000 ~ profit_bin, data = cleanCEO, cex.axis = .7,
        main = "CEO Salaries by Company Profit buckets",
        ylab = "Salary(millions dollars)",
        xlab = "Profits(millions dollars)")
```
From the graph above, we notice an upward trend in median salary with increasing profits in the realm of positive profits . There are some outliers with high salaries in the zero to 250 million bucket. The wider range of salaries in the zero to 250 million as well as greater than 250 million bucket is also noticeable. 
It is also interesting to note that CEOs in companies with negative profits did not get paid the lowest. Infact, the median value in that bucket is slightly higher than the 0 to 250 million bucket. Since there are only 10 negative profit values in the dataset, not sure how much weightage should be given to this.
