...
### Relationship between mktval and Salary using boxplots
...
CEO Tenure, reported in years are continuous integer values from 0 to 37. Before analysing the influence of ceo tenure on the primary variables, we group them into bins for easier analysis.
...
We calculate the mean and median values for salary for each of these ceo tenure bins and see if there is any particular pattern visible.
...
Now we analyse if CEO Tenure and market values are related.
...
Based on these graphs, CEO tenure does not play a significant role in explaining the difference in salary across various market value groups.
...
```
```{r}
mktval_mean_by_grad = by(cleanCEO$mktval/1000, grad_bins, mean)
mktval_median_by_grad = by(cleanCEO$mktval/1000, grad_bins, median)

plot(sort(unique(grad_bins)),mktval_mean_by_grad,
     main = "Plot of mean market value by education",
     xlab = "Graduated or Not",
     ylab = "Market value in $ millions")

plot(sort(unique(grad_bins)),mktval_median_by_grad,
     main = "Plot of median market value by education",
     xlab = "Graduated or not",
     ylab = "Market value in $ millions")
```
